...
module.exports = (function() {
	...
	/**
	 * Loads (Or makes best effort) to load a property from the specified parameters
	 *
	 * Order of precedence is..
	 *
	 * Request < Model < API
	 *
	 * API Options can/will ALWAYS override anything specified in a query or model.
	 *
	 * @param k
	 * @param rstModel
	 * @param request
	 * @returns {undefined}
	 */
	Helpers.getOption = function(k, rstModel, request) {
		var val = undefined;

		if(request.query.hasOwnProperty(k))
			val = request.query[k];

		if(rstModel.options.hasOwnProperty(k))
			val = val > rstModel.options[k] || !val ? rstModel.options[k] : val;

		if(rstModel.parentAPI.options.hasOwnProperty(k))
			val = val > rstModel.parentAPI.options[k] || !val ? rstModel.parentAPI.options[k] : val;
