var express     = require('express'),
	...
	scaffold    = require('./model_scaffold');
	...
	var RestModel = function(options) {
		...
		this.extra = {
			params: _.filter(_.flatten(_.zip(
				options.extra && options.extra.params || [],
				[
					defaults.primaryKeyParameter(this)
				]), false), removeUndefined),
			get: _.filter(_.flatten(_.zip(
				options.extra && options.extra.get || [],
				[
					defaults.getByPrimaryKey(this)
				]), false), removeUndefined),
			post: _.filter(_.flatten(_.zip(
				options.extra && options.extra.post || []),
				[
					defaults.createNew(this)
				], false), removeUndefined),
			put: _.filter(_.flatten(_.zip(
				options.extra && options.extra.put || [],
				[
					defaults.updateRow(this)
				]), false), removeUndefined),
			patch: _.filter(_.flatten(_.zip(
				options.extra && options.extra.patch || [],
				[
			...
			delete: _.filter(_.flatten(_.zip(
				options.extra && options.extra.delete || [],
				[
					defaults.deleteWhere(this),
					defaults.deleteByPrimaryKey(this)
				]), false), removeUndefined)
		};
