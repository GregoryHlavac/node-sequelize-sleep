...
module.exports = (function() {
	 ...
	 * @constructor
	 */
	var Sleep = function(options){
		this.restModels = [];
		this.options = {};

		if(options)
			this.options = Helpers.mergeObjects(this.options, options);
	};

	Sleep.QueryHelpers = require('./helpers/QueryHelpers');

	Sleep.Helpers = Helpers;
	 ...
	 */
	Sleep.prototype.createAPI = function(models, authenticationCallback) {
		...
		if(_.isArray(models))
		{
			_.forEach(models, function(model) {
				if(_.isObject(model))
				{
					...
					this.restModels[rm.model.name] = rm;
				}
			}, this);
		}
