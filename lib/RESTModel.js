...
module.exports = (function() {
	...
	RestModel.prototype.getAssociationOptions = function(forName) {
		if(!this.options.associationOptions) return undefined;
		return this.options.associationOptions[forName];
	};
	...
	RestModel.prototype.bindModel = function(rt) {
		...
		if(this.options.walk_associations && this.options.walk_depth > 0)
		{
			...
			_.forEach(hmAssociations, function(value) {
				var asModel = new AssociationModel(this.parentAPI, this, { model: value }, depth);
				asModel.bindModel();
				children.unshift(asModel);
			}, this);
		}

		_.forEach(this.routes, function(value, key) {
			_.forEach(this.routes[key], function(scaffold) {
				var rt = scaffold;
				...
				if(rt.hasOwnProperty('auth'))
					this.modelRouter[key](rt.path, rt.auth, rt.callback);
				else
					this.modelRouter[key](rt.path, rt.callback);
			}, this);
		}, this);
